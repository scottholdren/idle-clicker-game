...
export class GameEngine implements IGameEngine {
  ...
  public performClick(): Decimal {
    ...
    clickValue = multiply(clickValue, state.engagement)
  ...
  /**
   * Calculate engagement cost for next level
   ...
   * Level 4: 500 SP
   */
  public getEngagementCost(currentLevel: number): number {
    const nextLevel = currentLevel + 1
    return 50 * (nextLevel * (nextLevel + 1) / 2)
  }

  /**
   * Check if player can afford engagement upgrade
   */
  public canAffordEngagement(): boolean {
    const state = this.getGameState()
    const cost = this.getEngagementCost(state.engagement)
    return state.prestigePoints.greaterThanOrEqualTo(decimal(cost))
  }

  /**
   * Purchase engagement level with SP
   */
  public purchaseEngagement(): boolean {
    const state = this.getGameState()
    
    if (!this.canAffordEngagement()) {
      return false
    }
    ...
    // Deduct SP
    state.prestigePoints = state.prestigePoints.minus(cost)
  ...
  /**
   * Update engagement growth based on SP (Dimension 1 only)
   * DEPRECATED: Engagement is now purchased with SP, not grown automatically
   */
  private updateEngagementGrowth(deltaTime: number): void {
    // No longer used - engagement is purchased with SP
    return
  }
  ...
  public updateIdleProgress(deltaTime: number): Decimal {
    ...
    // Apply engagement multiplier to idle generation
    earnings = multiply(earnings, state.engagement)
    ...
    const efficiency = calculateViewToClickEfficiency(state.totalEarned, strategyBonus, state.engagement)
  ...
  private updateAutomation(deltaTime: number): void {
    ...
    for (const automation of state.automationSystems) {
      if (automation.owned > 0) {
        ...
        // Apply engagement multiplier to automation
        const finalClicksThisUpdate = multiply(bonusClicksThisUpdate, state.engagement)
        
        // Perform automated clicks
        const totalClicks = Math.floor(finalClicksThisUpdate.toNumber())
  ...
  public performPrestige(): void {
    ...
    const newState: GameState = {
      ...
      temporaryEffects: [], // Clear temporary effects on prestige
  ...
  public calculatePrestigeGain(): Decimal {
    const state = this.getGameState()
    // Formula: (totalEarned / threshold)^0.6 where threshold varies by mode
    const threshold = getPrestigeThreshold()
    const gain = state.totalEarned.dividedBy(threshold).pow(0.6).floor()
    return gain.greaterThan(ZERO) ? gain : ONE
  }

  /**
   * Check if player can shift to Dimension 2
   */
  public canShiftToDimension2(): boolean {
    const state = this.getGameState()
    return state.currentDimension === 1 && state.engagement > 1
  }

  /**
   * Shift to Dimension 2 (resets Dim 1 progress, keeps engagement)
   */
  public shiftToDimension2(): boolean {
    const state = this.getGameState()
    
    if (!this.canShiftToDimension2()) {
      return false
    }
    
    // Reset Dimension 1 progress (keep engagement)
    const newState: GameState = {
      ...state,
      currentDimension: 2,
      
      // Reset Dim 1 currencies
      currency: ZERO,
      views: ZERO,
      totalClicks: 0,
      totalEarned: ZERO,
      
      // Reset Dim 1 progression
      baseClickValue: ONE,
      clickMultiplier: ONE,
      idleMultiplier: ONE,
      prestigePoints: ZERO,
      ...
      // Reset generators and upgrades
      idleGenerators: state.idleGenerators.map(g => ({ ...g, owned: 0 })),
      upgrades: state.upgrades.map(u => ({ ...u, currentPurchases: 0 })),
      purchasedUpgrades: new Set<string>(),
      temporaryEffects: [],
      
      // Keep engagement (this is the permanent carry-over)
  ...
  /**
   * Return to Dimension 1 from Dimension 2
   */
  public returnToDimension1(): boolean {
    const state = this.getGameState()
    ...
    // Reset to Dimension 1 (everything resets except engagement)
    const newState: GameState = {
      ...state,
      currentDimension: 1,
